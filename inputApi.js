...
const gamepad = {
  on: function (event, callback, index) {
    if (event == "button") {
      setInterval(() => {
        if (index == undefined) {
          for (let i = 0; i < 3; i++) {
            const btn = gamepad.input(i).buttons;
            Object.keys(btn).forEach((e, i) => {
              Object.values(btn)[i] == true;
              callback({ index: i, button: e });
            });
          }
        } else {
          const btn = gamepad.input(index).buttons;
          Object.keys(btn).forEach((e,i)=>{
            if(Object.values(btn)[i]){
              callback({index:index,button:e})
            }
          })
        }
      });
...
export { gamepad, mouseInput, getKey };
